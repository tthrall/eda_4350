...
We see that the `kmeans()` clusters are distinct from but similar to the groups formed above whose mean values served as initial cluster centers.  Here's a breakdown of observations belonging to each combination of $g_{x,y}$ group and `kmeans()` cluster.
