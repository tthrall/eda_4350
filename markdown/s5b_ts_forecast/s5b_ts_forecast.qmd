...
To remove a seasonal component one can apply a variant of $\nabla$, namely seasonal differencing defined as $\mathcal{I} - \mathcal{B}^\sigma$, where $\sigma$ is the number of time indices that span one season.[^txt-seasonal-diff]
...
In addition to removing non-random trends in the data, the differencing operator $\nabla$ can also remove a slowly varying random drift.  Consider the following model.
...
The process $Y(\cdot)$ is a random walk whose variance increases over time and is thus non-stationary, so that $X(\cdot)$ is also non-stationary.  But $\nabla \; X(\cdot)$ is stationary.
