...
Repeated application of $\mathcal{D}$ removes polynomial trends.
...
The $AR(p)$ and $MA(q)$ models can be combined into the following $ARMA(p,q)$ model.  To simplify notation we suppose for the moment that $\mu_X = 0$.  And once again $W(\cdot)$ denotes a white noise process.
...
Here we must impose a restriction on the polynomials $\phi(\cdot)$ and $\theta(\cdot)$ in addition to requiring their respective roots to all have magnitude greater than unity.  The additional requirement is that they have no roots in common.
...
$$
\begin{align}
  X(t) &= \alpha \; X(t - 1) \; + \; W(t) \; - \; \alpha \; W(t - 1) \\   \\
  \text{so that } \\ 
  \\ 
  p(\mathcal{B}) \; X(\cdot) &= p(\mathcal{B}) \; W(\cdot) \\   \\
  \text{where } \\ 
  \\ 
  p(z) &= 1 - \alpha \; z \quad \text{with } |\alpha| < 1
 \end{align}
$$ {#eq-ARMA_1_1_not}

The operator $p(\mathcal{B})$ can be cancelled from both sides of the equation, leaving only a white noise process: $X(\cdot) = W(\cdot)$.  Thus we assume that any polynomial factors common to both $\phi(\cdot)$ and $\theta(\cdot)$ have been eliminated.  This implies that the roots of $\phi(\cdot)$ are distinct from those of $\theta(\cdot)$.

The $ARMA(p,q)$ model can be expressed in causal form.[^ARMA_p_q_causal]

[^ARMA_p_q_causal]: That is, the rational function $\theta(z) \; \div \phi(z)$ can be represented as a power series obtained as the power series for $\phi(z)^{-1}$ multiplied by the polynomial $\theta(z)$.
