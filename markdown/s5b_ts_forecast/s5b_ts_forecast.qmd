...
A running average (also called a moving average) of a random process $X(\cdot)$ is a new process $Y(\cdot)$ formed as a local average of $X$ values.  A _weighted_ running average, with weights $\{ w_u \}_u$ takes the following form.
...
As an example, @fig-gt-lo-ma5 shows a 5-year _simple_ moving average 
...
Now, the $AR(p)$ model is defined to be that of a _stationary_ process $X(t)$, which constrains the coefficients of polynomial $\phi(\cdot)$.  Additional requirements that the model be _causal_[^causal] and _invertible_[^invertible] restrict attention to polynomials $\phi(\cdot)$ whose roots are all greater than unity in magnitude.  This ensures that the multiplicative inverse function $\phi (z)^{-1}$ is well defined and has a power series expansion for $|z| \le 1$.[^poly-roots]
...
Consequently we have the following causal form of the model.[^txt-causal-form] [^ARp-causal]

[^txt-causal-form]: See @shumway2025 [page 117].

[^ARp-causal]: To expand $\phi(z)^{-1}$ in a power series, define polynomial $f(\cdot) = 1 - \phi(\cdot)$ so that $f(z) = \phi_1 z + \cdots + \phi_p z^p$.  We then have $\phi(z)^{-1} = (1 - f(z))^{-1}$ as a geometric series in $f(z)$ which can be re-expressed as a power series in $z$.
...
Consider the following two MA(1) models.[^txt-MA1-equivalence]

[^txt-MA1-equivalence]: See @shumway2025 [page 92].
...
The probability distributions of the two processes $X(\cdot)$ and $Y(\cdot)$ are identical[^MA1-transform], so anyone attempting to identify the respective models could not distinguish them based on their realizations as time series data.

[^MA1-transform]: Transform the second equation into the first as follows. For all time indices $\nu$ set $U(\nu) = 5 V(\nu - 1)$, and then reverse time by setting $Y'(\nu) = Y(- \nu)$ and $U'(\nu) = U(- \nu)$.
...
The PACF turns out to have the following form[^txt-MA1-PACF].

[^txt-MA1-PACF]: See @shumway2025 [page 111].
...
Consider for example the following putative $ARMA(1,1)$ model.[^txt-ARMA-1-1-not]

[^txt-ARMA-1-1-not]: See @shumway2025 [page 95].
...
The $ARMA(p,q)$ model can be expressed in causal form as follows.[^txt-ARMA-p-q-causal] [^ARMA-p-q-causal]

[^txt-ARMA-p-q-causal]: See @shumway2025 [page 97].

[^ARMA-p-q-causal]: The rational function $\theta(z) \; \div \phi(z)$ can be expressed as a power series, namely the power series of $\phi(z)^{-1}$ multiplied by the polynomial $\theta(z)$.
...
It is also useful to invert the MA operator in an ARMA model to obtain the white noise process $W(t)$ as a linear function of current and past values of $X(t)$.[^txt-ARMA-p-q-inv] [^MAq-inv]

[^txt-ARMA-p-q-inv]: See @shumway2025 [page 97].

[^MAq-inv]: This inversion requires each root of polynomial $\theta(\cdot)$ to have magnitude greater than unity.
...
In vector-matrix notation we have the following linear system.[^txt-Gamma-defn]

[^txt-Gamma-defn]: @shumway2025 [page 114] introduce the $\Gamma$ matrix.
...
$\Gamma$ is a covariance matrix, and is therefore non-negative definite.  In fact $\Gamma$ is positive definite (non-singular) unless $X(\cdot)$ is a degenerate process[^degenerate-process].  In any case $\hat{\beta}_\bullet$ can be defined by applying the inverse (or a generalized inverse) of $\Gamma$ to $\tilde{\gamma} [\cdot]$.

[^degenerate-process]: By "degenerate process" we mean a process $X(\cdot)$ for which there is a filter $a(u)$, equal to zero for all but finitely many lags $u$, that annihilates $X(\cdot)$: $a * (X - \mu_x) = 0$.

As noted above, we have assumed for the sake of discussion that the entire auto-covariance function $\gamma_X (\cdot)$ is known.  In practice such statistics must be estimated from the data.  But direct estimation is not possible, due to the restricted range of differences in data indices.[^direct-estimates]  Instead, we must rely on a model of the process for such estimates.

[^direct-estimates]: Any two time indices within the $T$ observations can differ by no more than $T-1$.  Yet the needed estimates include $\gamma_X(T + u - 1 - j)$, with $u \ge 1$ and $0 \le j < T$.  For $u = 1$ and $j = 0$ we have $\gamma_X(T)$, which exceeds the limits of direct estimation.
...
We adopt the following notation for the expectation of $\mathcal{N}(u + t_f)$, the process at a future time, conditioned on $\{ \mathcal{N}(s) \}_{s \le t_f}$, the history of the process prior to and including the final observation.[^horizon_h]

[^horizon_h]: When attempting to predict future values $\mathcal{N} (u + t_f)$ from data values indexed by $t \in (t_0, \ldots, t_f)$ the index symbol $h$ is often used in place of the generic indexing difference $u$ to denote forecast _horizon_.
...
Setting $k = 0$ we obtain the following expected square of $R(u + t_f \; | \; \wp (t_f))$, called mean squared prediction error.[^caveat-msep]

[^caveat-msep]: Recall that the formula given here for the mean squared error of prediction assumes that the coefficients of the model and the parameters of the process are known.  In applications these quantities must be estimated, which increases mean squared prediction error.
...
### Truncated Predictors[^txt-trunc-ARMA-predictor]

[^txt-trunc-ARMA-predictor]: @shumway2025 [page 119]
...
### ARMA(1, 1) Simulation[^txt-ARMA-1-1-xmpl]

[^txt-ARMA-1-1-xmpl]: See @shumway2025 [page 120].
...
The forecast variance can be approximated from @eq-var-predictor-resid using $\psi$ coefficients that turn out to satisfy [^txt-ARMA-psi] [^ARMA-1-1-psi]

[^txt-ARMA-psi]: See @shumway2025 [page 103].

[^ARMA-1-1-psi]: The formula is obtained by expanding $\frac{1 + \theta_1 z}{1 - \phi_1 z}$ in a power series.
...
We now set $\phi_1 = \theta_1 = 0.5$ in order to simulate 150 observations from this model and forecast the last 50 of the 150 observations.  This example uses `astsa` functions `sarima.sim()` and `sarima.for()`.[^tp-1-1-sim]

[^tp-1-1-sim]: The simulation code is based on a similar example in [Fun with asta](https://github.com/nickpoison/astsa/tree/master/fun_with_astsa).
...
The process $Y(\cdot)$ is a _random walk_ whose variance increases over time and is thus non-stationary.  Therefore $X(\cdot)$ is also non-stationary.  But applying the differencing operator $\nabla$ to $X(\cdot)$ induces stationarity.[^ARIMA-1-1-0-xmpl]

[^ARIMA-1-1-0-xmpl]: From covariance calculations, $\nabla \; W(\cdot)$ is an $AR(1)$ process, which makes $\nabla \; X(\cdot)$ an $AR(1)$ process.
...
The coefficient $\alpha$ is called _drift_ in this context.

The truncated predictor at $u = 1$ (called the _one step ahead forecast_) can be determined as follows.  Consider the case in which both $X(\cdot)$ and $W(\cdot)$ are Gaussian processes.  This enables us to use conditional expectations to derive the form of the best linear predictor in the non-Gaussian case.  We have 
...
### EWMA Example[^txt-EWMA-xmpl]

[^txt-EWMA-xmpl]: See @shumway2025 [p. 148].

The following ARIMA(0,1,1) model has been applied to many economic time series.
...
The model is also referred to as an integrated moving average model, or IMA(1, 1).  The model leads to a frequently used forecasting method called exponentially weighted moving averages (EWMA) or simple exponential smoothing (SES).
...
The linear prediction of the current value based on past values is thus the same exponential smoothing.
...
This gives the following linear recursion formula.
...
That is, the current one step ahead predictor is a convex combination of the most recent observation and the preceding one step ahead predictor.

The truncated one step ahead predictor based on the data follows a similar recursion.
...
\begin{align}
  ...
  \begin{cases}
    X(t_0) & \text{for } t = t_0 \\ 
    (1 - \lambda) \; X(t) + \lambda \breve{X}(t \; | \; s \le t-1) & \text{for } t_0 < t \le t_f
  \end{cases}
\end{align}
...
Now, based on the $IMA(1, 1)$ model we have 
...
Exponential smoothing with trend and seasonality (ETS) is a modeling framework, distinct from ARIMA, based on Holt-Winters forecasting, which is widely used in economic analyses.[^art-Holt-Winters]

[^art-Holt-Winters]: See @holt1957forecasting and @winters1960forecasting.

In this framework the forecast $\breve{X}(u + t \; | \; s \le t)$ consists of three components: level $\ell (t)$; local trend $\mathit{b} (t)$; and seasonality $\mathit{s} (u + t \; | \; s \le t)$, with $s, t \in \tau_T$.  The forecast equation expresses $\breve{X}(u + t \; | \; s \le t)$ as a linear combination of these components, and each component has its own linear equation.  Together these equations define a _state-space_ model.

In this section we merely outline the ETS framework.  For further details see the cited resources.  Also, note that `R` function `fable::ETS()` selects and fits the models described in this section with modest guidance from the user.

#### Level component

We begin with simple exponential smoothing (SES), which entails the level component $\ell(\cdot)$ alone.  We re-express the one step ahead forecast recursion from the previous section (@eq-tp-one-step-recur) by setting $\alpha = 1 - \lambda$, and restricting $\alpha$ to the open unit interval.
...
\begin{align}
  ...
  \begin{cases}
    X(t_0) & \text{for } t = t_0 \\ 
    \alpha \; X(t) + (1 - \alpha) \; \breve{X}(t \; | \; s \le t-1) & \text{for } t_0 < t \le t_f
  \end{cases} \\ 
  \\ 
...
From the previous section we know that a forecast beyond the data is simply the final data value.

$$
\begin{align}
  \breve{X}(u + t_f \; | \; \tau_T) &= X(t_f) \quad \text{for } u > 1
\end{align}
$$ {#eq-ets-ANN-flatline}

The state-space or component form of this recursion is as follows (with boundary conditions to be set for $\ell (\cdot)$).

$$
\begin{align}
  \breve{X}(u + t \; | \; s \le t) &= \ell (t) \\
...
$$
\begin{align}
  \breve{X}(u + t \; | \; s \le t) &= \ell (t) \; + \; \mathit{b}(t) \times u \\
...
This above forecast system is known as Holt's trend method.  Over the years a modification has been introduced to dampen the effect of the trend term $\mathit{b}(t)$ on long-term forecasts, via damping coefficient $\phi \in (0, 1)$, as follows.
...
Starting with @eq-ets-AAN-ss, we now add a seasonal component, $\mathit{s}(u + t \; | \; s \le t)$, where a fixed positive integer $\sigma$ denotes the number of successive time indices comprising a single season.  For $t, (u + t) \in \tau_T$ we fit $\mathit{s}(u + t \; | \; s \le t)$ recursively.  Then for $(u + t) > t_f$ we refer back to the most recent fitted value $\mathit{s} (\tilde{t})$ such that $\tilde{t} \equiv (u + t) \bmod \sigma$.

$$
\begin{align}
  \breve{X}(u + t \; | \; s \le t) &= \ell (t) \; + \; \mathit{b}(t) \times u \; + \; \mathit{s}(u + t \; | \; s \le t) \\
  \ell (t) &= \alpha \; (X(t) - \mathit{s}(t - \sigma)) + (1 - \alpha) \; \left ( \ell(t - 1) + \mathit{b}(t - 1) \right ) \\ 
  \mathit{b} (t) &= \beta \; \left ( \ell(t) - \ell(t - 1) \right ) \; + \; (1 - \beta) \; \mathit{b}(t - 1) \\ 
  \mathit{s} (t) &= \gamma \left ( X(t) - \ell (t-1) - \mathit{b} (t-1) \right ) \; + \; (1 - \gamma) \mathit{s}(t - \sigma) \\ 
  \\ 
  &\text{where } \alpha, \beta, \gamma \in (0, 1)\\ 
  \\ 
  &\text{and where } \\ 
  & \mathit{s}(u + t \; | \; s \le t) = \mathit{s} (\tilde{t}), \text{ with} \\ 
...
Each ETS model can be categorized according to the status of each potential model component: level, trend, and season.[^art-ETS-categories]

[^art-ETS-categories]: See @hyndman2002state.

The status of a component is one of the following:

  - A: used as an additive component
  - Ad: damped or otherwise adjusted
  - M: used as a multiplicative component
  - N: not used

Here are some examples of this identification of ETS model-type.
...
Note: @hyndman2021forecasting recommend against certain combinations that may lead to numerical instability when estimating parameters.  For example they recommend using $ETS(M, *, M)$ rather than $ETS(A, *, M)$.  Much of this guidance is contained in the documentation (and curated options) for function `fable:ETS()`.

### CO2 Example[^txt-CO2-Mauna-Loa]

[^txt-CO2-Mauna-Loa]: See @shumway2025 [p. 163].

## Closing Remarks

This technical introduces families and examples of models used to forecast time series data.  The presentation is based on @shumway2025 along with the supporting `R` package `astsa` [@astsa].[^2]

[^2]: Another textbook, @brillinger2001, treats a time series as a function of time, denoted $X(t)$, which is the practice followed in this note.

This introduction does not address the building and evaluation of time series models, leaving that for another time.  Here are some online resources for those interested.
