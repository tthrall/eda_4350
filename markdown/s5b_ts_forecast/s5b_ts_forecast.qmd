...
If $X, Y, Z_\bullet$ have a joint normal distribution, then the conditional expectations above are linear combinations of the components of $Z_\bullet$.  The _partial_ correlation is defined by setting $\hat{X}, \hat{Y}$ equal to the linear combinations that minimize mean squared error.
...
$$
\begin{align}
  \rho_{X, Y | Z_\bullet} &= cor \left (X - \hat{X} \; , \; Y - \hat{Y} \right)
\end{align}
$$ {#eq-cor-X-Y-given-Z}

In the time series context the PACF is defined with reference to a univariate, stationary Gaussian process, $\mathcal{N}(t)$, having zero mean.  Unlike the white noise process $W(t)$, we do not assume $\mathcal{N}(t)$ to be free of auto-correlation.  Recall that for a univariate process the auto-covariance and auto-correlation functions are even, e.g., $\rho (-u) = \rho (u)$.
...
The coefficients in the linear combination are shared by $\hat{\mathcal{N}}(t)$ and $\hat{\mathcal{N}}(t + u)$ but are applied in reverse order.  This is because the time-reversed process $\mathcal{N}(-t)$ has the same auto-covariance structure as the original process $\mathcal{N}(t)$.
