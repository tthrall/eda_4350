...
$$
\begin{align}
  \tilde{\mathcal{N}} (u \; | \; \wp (t_f)) &= E \left \{ \mathcal{N} (u + t_f) \; | \; \mathcal{N} (s), \; s \le t_f \right \}
\end{align}
...
$$
\begin{align}
  \hat{\mathcal{N}} (u \; | \; \wp (t_f)) &= \tilde{\mathcal{N}} (u \; | \; \wp (t_f))
\end{align}
...
$$
\begin{align}
  R(k \; | \; \wp (t_f)) &= \mathcal{N} (k + t_f) \; - \; \tilde{\mathcal{N}} (k \; | \; \wp (t_f)), \quad k \ge 1
\end{align}
$$ {#eq-predictor-resid-defn}

Combining @eq-predicted-AR-infty and @eq-predictor-inv-form-u we can express process $R(u \; | \; \wp (t_f))$ for $u \ge 1$ in a finite auto-regressive form (that expands with $u$).

$$
\begin{align}
  R(u \; | \; \wp (t_f)) &= \mathcal{N} (u + t_f) \; - \; \tilde{\mathcal{N}} (u \; | \; \wp (t_f)) \\ 
  &= W(u + t_f) - \sum_{\nu = 1}^{u - 1} \; \pi_\nu \; R(u - \nu\; | \; \wp (t_f))
\end{align}
$$ {#eq-predictor-resid-AR}

Similarly, combing @eq-predicted-causal-form and @eq-predictor-causal-form-u we can express $R(u \; | \; \wp (t_f))$ as the following finite moving average of the white noise terms (where the number of terms again increases with $u$).
...
$$
\begin{align}
  E \left \{ R(u \; | \; \wp (t_f)) \times R(u + k \; | \; \wp (t_f)) \right \} &= \sigma_W^2 \; \sum_{\nu = 0}^{u - 1} \; \psi_\nu \; \psi_{\nu + k}
\end{align}
...
Using the entire history of the process $\{ \mathcal{N} (s), \; s \le t_f \}$ preceding and including the observed values, we've expressed the linear predictor $\tilde{\mathcal{N}} (u \; | \; \wp(t_f))$ as an infinite moving average (@eq-predictor-causal-form-u) and, alternatively, as an infinite auto-regression (@eq-predictor-inv-form-u).
...
$$
\begin{align}
  \breve{W} (t) &= \mathcal{N}(t) \; - \; \phi_1 \; \mathcal{N}(t - 1) \; - \; \theta_1 \; \breve{W} (t - 1) \\ 
  \\ 
  &\text{for } t \in \tau_T, \text{ and setting } \\
  \\
  0 &= \mathcal{N} (t_0 - 1) = \breve{W} (t_0 - 1)
\end{align} 
$$ {#eq-ARMA-1-1-init}
...
The truncated predictor at $u = 1$ (called the _one step ahead forecast_) can be determined as follows.  Consider the case in which both $X(\cdot)$ and $W(\cdot)$ are Gaussian processes.  This enables us to use conditional expectations, which yields the form of the best linear predictor in the non-Gaussian case.  We have 
...
The model is also referred to as an integrated moving average model, or IMA(1, 1).  The model leads to a frequently used forecasting method called exponentially weighted moving averages (EWMA).
...
$$
\begin{align}
  \hat{X}(1 \; | \; \wp (t-1)) &= (1 - \lambda) \; X(t - 1) \; + \; (1 - \lambda) \; \sum_{\nu = 2}^\infty \lambda^{\nu - 1} \; X(t - \nu) \\ 
  &= (1 - \lambda) \; X(t - 1) \; + \; (1 - \lambda) \; \sum_{\nu = 1}^\infty \lambda^\nu \; X(t -1 - \nu) \\ 
  &= (1 - \lambda) \; X(t - 1) \; + \; \lambda \left \{ (1 - \lambda) \; \sum_{\nu = 1}^\infty \lambda^{\nu - 1} \; X(t -1 - \nu) \right \}  \\ 
  &= (1 - \lambda) \; X(t - 1) \; + \; \lambda \; \hat{X}(1 \; | \; \wp (t-2)) 
\end{align}
$$
...
\begin{align}
  ...
  \begin{cases}
    X(t_0) & \text{for } u = 1 \\ 
    (1 - \lambda) \; X(u - 1 + t_0) + \lambda \breve{X}(u -1 + t_0 \; | \; \tau_{u-1}) & \text{for } 1 < u < T
  \end{cases}
\end{align}
$$
...
To forecast beyond the data we assume for the moment that $X(\cdot)$ and $W(\cdot)$ are Gaussian process in order to derive least squares predictors form conditional expectations.
...
@fig-EWMA-plus-wn shows a simulation of the IMA process $X(\cdot)$, along with the one step ahead exponential smoothing of past values.  We set $\lambda =$ `r EWMA_sim_params$ lambda`, equivalent to an EWMA smoothing parameter of $\alpha = 1 - \lambda =$ `r EWMA_sim_params$ alpha`.

```{r}
#| label: fig-EWMA-plus-wn
...
# process with exponential smoothing
# yields estimate of smoothing parameter alpha
x.ima <- stats::HoltWinters(x, beta=FALSE, gamma=FALSE)

# create tibble as input to gglot()
ima_tbl <- tibble::tibble(
  time  = 1:nrow(x.ima$ fitted), 
  X     = x.ima$ x [time], 
  X_hat = x.ima$ fitted [, 1]
) |> 
  tidyr::pivot_longer(
    cols = X:X_hat, 
    names_to = "variable", 
...
This introduction does not address the building and evaluation of time series models, leaving that for another time.  Here are some additional resources for those interested.
