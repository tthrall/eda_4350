...
Both societies and natural phenomena follow cycles determined by earth's daily rotation and annual revolution about the sun.  Other cycles also arise naturally or through societal conventions.  In modeling a random process one can account for these cycles in several ways.  In this section we introduce methods and notation for incorporating seasonality into ARIMA models.

As in @sec-ETS we denote by $\sigma$ the number of consecutive time indices that comprise one season (or cycle).  $ARMA(P, Q)_\sigma$ (with capital letters and a $\sigma$ subscript) denotes the following form of pure seasonal ARMA model.
