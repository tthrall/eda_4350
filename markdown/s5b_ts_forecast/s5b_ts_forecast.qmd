...
Let $X(\cdot)$ be a stationary process, with observed values for 
...
For $u \ge 1$ let $\hat{X} (u | \tau_T)$ denote the _linear least squares predictor_ of $X(u + t_0 + T - 1)$ as an affine function of $\{ X(t) \}_{t \in \tau_T}$.

$$
\begin{align}
  (\hat{\alpha}, \hat{\beta}_\bullet) &= \arg \min_{\alpha, \beta_\bullet} \; MSE \left \{ X(u + t_0 + T - 1), \; \alpha + \sum_{\nu = 0}^{T-1} \beta_\nu X(t_0 + \nu) \right \} \\ 
...
We assume for the moment that the first and second-order statistics (mean, variance, auto-covariance, ...) of $X(\cdot)$ are known in order to express the linear coefficients $\hat{\alpha}, \hat{\beta}_\bullet$ as functions of those statistics.

Consider the expression for mean-squared error (MSE).

$$
\begin{align}
  MSE \left \{ X(u + t_0 + T - 1), \; \alpha + \sum_{\nu = 0}^{T-1} \beta_\nu X(t_0 + \nu) \right \} \\ 
  = E \left \{ \left ( X(u + t_0 + T - 1) \; - \alpha - \sum_{\nu = 0}^{T-1} \beta_\nu X(t_0 + \nu) \right )^2 \right \}
\end{align}
$$ {#eq-predictor-MSE}
...
$$
\begin{align}
  0 &= E \left \{ \left ( X(u + t_0 + T - 1) \; - \hat{\alpha} - \sum_{\nu = 0}^{T-1} \hat{\beta}_\nu X(t_0 + \nu) \right ) \right \} \\ 
  &= \mu_X - \hat{\alpha}  \; - \; \sum_{\nu = 0}^{T-1} \hat{\beta}_\nu \; \mu_X
\end{align}
...
This technical note is intended as an overview of time series analysis based on @shumway2025 along with the supporting `R` package `astsa` [@astsa]. The reader should feel free to select the presented examples of greatest interest, skipping others to save time.
...
@fig-djia shows the trading day closings and returns (percent change)[^2] of the Dow Jones Industrial Average (DJIA, `astsa::djia`) from 2006 to 2016. It is easy to spot the financial crisis of 2008.

[^2]: The return is here calculated as $log_e$ of the ratio: current day's closing price divided by that of the preceding day. The median value is about 6 basis points.
