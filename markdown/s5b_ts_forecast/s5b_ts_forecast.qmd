...
By convention, $d \in \{ 0, 1, 2, \ldots \}$ so that $ARIMA(p, 0, q)$ is synonymous with $ARMA(p, q)$.  If $d \ge 1$ then $\mathcal{D}^d$ removes $\mu_X$, which can be omitted from the formula for simplificaton.

As previously noted the operator $\mathcal{D}^d$ removes any polynomial trend of degree $d$ or less.

## Forecasting

### Notation

Let $X(\cdot)$ be an $ARMA(p, q)$ process, with observed values for 
...
For $u \ge 1$ we denote as $\hat{X} (u | \tau_T)$ the linear least squares predictor of $X(u + t_0 + T - 1)$ as an affine function of $\{ X(t) \}_{t \in \tau_T}$.

$$
\begin{align}
  \hat{\beta}_\bullet &= \arg \min_{\beta_\bullet} \; MSE \left \{ X(u + t_0 + T - 1), \; \beta_0 + \sum_{\nu = 0}^{T-1} \beta_k X(t_0 + \nu) \right \} \\ 
  \hat{X} (u | \tau_T) &= \hat{\beta}_0 + \sum_{\nu = 0}^{T-1} \hat{\beta}_\nu X(t_0 + \nu) 
 \end{align}
$$
