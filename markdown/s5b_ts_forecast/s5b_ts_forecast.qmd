...
The distinction of time series analysis is that observations are indexed by time, and are not assumed to be statistically independent. Instead, time series data are typically modeled as a realization of a random process of the following form.[^real_valued]
...
[^real_valued]: We assume the components of $X_\bullet(t)$ to be real-valued, unless stated otherwise.
...
Going further, the _partial autocorrelation function (PACF)_ is a useful diagnostic tool.  Here's the idea [@shumway2025, page 108].  Recall that if $X$, $Y$, and $Z_\bullet = \{Z_1, \ldots, Z_k\}$ are random variables, then the conditional correlation between $X$ and $Y$ given $Z_\bullet$ is obtained by first regressing $X$ and $Y$ on $Z_\bullet$ to obtain predictors $\hat{X}$ and $\hat{Y}$.  If $X, Y, Z_\bullet$ have a joint normal distribution, we have
...
which are linear combinations of the components of $Z_\bullet$.  More generally, $\hat{X}, \hat{Y}$ are defined as the linear combinations of $Z_\bullet$ that minimize mean squared error.
...
In the time series context the PACF is defined with reference to a univariate, stationary Gaussian process, $\mathcal{N}(t)$, having zero mean.[^gauss_proc].

[^gauss_proc]: That is, we assume that $\mathcal{N}(t)$ is a stationary, mean-zero, autocorrelated process such that for any set of distinct time indices $t_1, \ldots, t_K$ the joint probability distribtuion of $(\mathcal{N}(t_1), \ldots), \mathcal{N}(t_K)$ is multivariate normal.
...
[^rho_even]: Recall that for a univariate process the auto-covariance and autocorrelation functions are even, e.g., $\rho (-u) = \rho (u)$.
...
According to this model[^cmort_trend] the pattern over time of $X_1(t)$ is captured by the mean over time, $m(t, X_{2:3}(t))$. Once the mean level is accounted for, the residual series $W(t) = X_1 - m(t, X_{2:3}(t))$ is modeled as a series free of autocorrelation.[^white_noise] This assumption seems plausible, since $X_1(t)$ counts the number of arguably independent events (cardiovascular deaths) in each week.
...
[^white_noise]: The authors generally use the notation $W(t)$ to signify white noise, an uncorrelated stationary random process having mean 0.  Moreover, the authors typically reserve this notation to signify an uncorrelated stationary _Gaussian_ process, that is, a process of independent and identically distributed ("iid") normal variables having mean 0.
...
Auto-regressive models are based on the idea that the current value of the series, $X(t)$, can be approximated, or forecast, as a function of $p$ past values, $X(t−1), X(t−2), \ldots, X(t−p)$, which leads to the following definition of an auto-regressive model of order $p$, abbreviated as $AR(p)$.
...
and where $W(t)$ again denotes a residual "white noise" stationary process: free of autocorrelation and having zero mean.  (Even more, $W(t)$ is often assumed to be a sequence of independent and identically distributed, or "iid", normal variates having zero mean.)

Equivalently, using the back-shift operator $\mathcal{B}$ the $AR(p)$ model can be expressed as follows.  
...
The polynomial $\phi(\cdot)$ applied to the back-shift operator $\mathcal{B}$ is an _auto-regressive operator of order $p$_.
...
@fig-faithful below represents eruptions of the "Old Faithful" geyser in Yellowstone National Park, Wyoming from August 1 to August 15, 1985.[^3] The `delay` variable denotes the number of minutes since the previous eruption and the `duration` variable denotes the duration of the eruption, again in minutes.

[^3]: There are two prominent `R` packages representing the Old Faithful measurements during August, 1985. The `MASS::geyser` data set of 299 observations [@MASS] includes nocturnal measurements whose duration was coded as 2, 3 or 4 minutes, having originally been described as ‘short’, ‘medium’ or ‘long’. The `datasets::faithful` data set of 272 measurements, shown in the figure, excludes 27 of these nocturnal measurements.
...
@fig-djia shows the trading day closings and returns (percent change)[^5] of the Dow Jones Industrial Average (DJIA, `astsa::djia`) from 2006 to 2016. It is easy to spot the financial crisis of 2008.

[^5]: The return is here calculated as $log_e$ of the ratio: current day's closing price divided by that of the preceding day. The median value is about 6 basis points.
...
The autocorrelation function is thus[^7]
...
The magnitude of such erroneous calculations can be represented by a quantity called the *effective sample size* (ESS). The calculation of ESS varies with application[^8], but here's the basic idea in the present context. Consider a finite sequence of *independent* draws from the distribution of the random variable $X(t_0)$, thereby producing a sample of independent observations. Define the effective sample size, $N_d$, to be the number of draws whose sample mean has the same variance as that of the autocorrelated case.
