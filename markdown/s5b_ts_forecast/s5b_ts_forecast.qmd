...
The linear prediction of the current value based on past values is thus the same exponential smoothing of past values.[^project-X-onto-its-past]

[^project-X-onto-its-past]: $\hat{X}(t \; | \; \wp (t-1))$ is the orthogonal projection of $X(t)$ onto the space spanned by $\{ X(s), s < t \}$.  See @shumway2025 [sec. B.1].
