...
$\Gamma$ is a covariance matrix, and is therefore non-negative definite.  In fact $\Gamma$ is positive definite (non-singular) unless $X(\cdot)$ is a degenerate process[^degenerate_process].  In any case $\hat{\beta}_\bullet$ can be defined by applying the inverse (or a generalized inverse) of $\Gamma$ to $\tilde{\gamma}_\bullet$.
...
We now examine the residuals from the linear least squares predictors defined above.  For fixed $u \ge 1$ the predicted variable is $\mathcal{N} (u + t_f)$, expressed below in both causal and inverted forms.
...
Let $R(\cdot \; | \; \mathcal{p} (t_f))$ denote the residuals of the predictors.
...
Combining @eq-predictor-inv-form-u and @eq-predicted-AR-infty we can express process $R(k \; | \; \mathcal{p} (t_f))$ for $k \ge 1$ in an expanding but finite auto-regressive form.
...
Similarly, combing @eq-predicted-causal-form and @eq-predictor-causal-form-u we can express $R(\cdot\; | \; \mathcal{p} (t_f))$ as the following finite but expanding moving average of the white noise terms.
...
From the last result we see that for fixed $u$ the residual random variable $R(u\; | \; \mathcal{p} (t_f))$ has zero mean and the following variance.
