...
abstract:
  "Introduce the components of time domain models and the forecasts of time series that such models enable.  <br><br>"
...
The distinction of time series analysis is that observations are indexed by time, and are not assumed to be statistically independent. Time series data are typically viewed as a realization of a random process of the following form.
...
#### Covariates and Trend may Suffice

Consider, for example, the cardiovascular mortality data @fig-cmort, where mortality is modeled as a response to temperature and the level of particulate matter. In this example we can formulate the expected value of mortality[^2] in the following terms.

[^2]: The authors consider various forms for the function $f(t)$: a linear function, a quadratic function, or a locally smoothed version of $X_1(t)$.
...
\begin{align}
  m(t, X_{2:3}(t)) 
   ...
   \\ 
    \text{ where } \\ 
    X_1(t) &= \text{mortality count in week } t \\ 
    X_2(t) &= \text{temperature } (F^\circ) \text{ in week } t \\ 
    X_3(t) &= \text{level of particulate matter in week } t \\ 
\end{align}
$$

Each of the three components of $X_\bullet (t)$ exhibits an annual periodicity, as one might expect. In the textbook [@shumway2025, page 55] the authors arrive at a model for $X_1(t)$ of the following form.
...
The point here is that the pattern over time of $X_1(t)$ is captured by the mean over time, $m(t, X_{2:3}(t))$. Once the mean level is accounted for, the residual series $W(t) = X_1 - m(t, X_{2:3}(t))$ is modeled as a series free of auto-correlation. This seems reasonable, since $X_1(t)$ counts the number of arguably independent events (cardiovascular deaths) in each week.

#### Auto-correlation may Persist

We now consider a contrasting example: the size of the fish population in response to the El Niño weather pattern (@fig-soi).
...
```{r}
#| label: fig-cmort
#| fig-cap: "Cardiovascular Mortality per Week in Los Angeles"
