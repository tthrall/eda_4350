...
In this case the 5-year moving average operation can be expressed as follows.[^ma-simple]

[^ma-simple]: The example illustrates a simple moving average. A weighted moving average may have non-negative coefficients that sum to unity. The term "moving average" is also used more broadly to mean a component of a time series model that filters an uncorrelated noise sequence.
...
To remove a seasonal component one can apply a variant of $\mathcal{D}$, namely seasonal differencing defined as $\mathcal{I} - \mathcal{B}^\sigma$, where $\sigma$ is the number of time indices that span one season.[^txt-seasonal-diff]
...
In addition to removing non-random trends in the data, the differencing operator $\mathcal{D}$ can also remove a slowly varying random drift.  Consider the following model.
...
The process $Y(\cdot)$ is a random walk whose variance increases over time and is thus non-stationary, so that $X(\cdot)$ is also non-stationary.  But $\mathcal{D} \; X(\cdot)$ is stationary.

$$
\begin{align}
  \mathcal{D} \; X(t) &= \mathcal{D} \; Y(t) + \mathcal{D} \; W(t) \\ 
  &= V(t) \; + \; \mathcal{D} \; W(t)
\end{align}
$$
...
This technical note presents an overview of time series forecasting based on @shumway2025 along with the supporting `R` package `astsa` [@astsa].[^2]
