...
It is also useful to invert the MA operator in an ARMA model to obtain the white noise process $W(t)$ as a linear function of current and past values of $X(t)$.[^txt_ARMA_p_q_inv] [^MAq_inv]

[^txt_ARMA_p_q_inv]: See @shumway2025 [page 97].

[^MAq_inv]: This inversion requires each root of polynomial $\theta(\cdot)$ to have magnitude greater than unity.
...
$\Gamma$ is a covariance matrix, and is therefore non-negative definite.  In fact $\Gamma$ is positive definite (non-singular) unless $X(\cdot)$ is a degenerate process[^degenerate_process].  In any case $\hat{\beta}_\bullet$ can be defined by applying the inverse (or a generalized inverse) of $\Gamma$ to $\tilde{\gamma} [\cdot]$.

[^degenerate_process]: By "degenerate process" we mean a process $X(\cdot)$ for which there is a filter $a(u)$, equal to zero for all but finitely many lags $u$, that annihilates $X(\cdot)$: $a * (X - \mu_x) = 0$.

As noted above, we have assumed for the sake of discussion that the entire auto-covariance function $\gamma_X (\cdot)$ is known.  In practice such statistics must be estimated from the data.  But direct estimation is not possible, due to the restricted range of differences in data indices.[^direct_estimates]  Instead, we must rely on a model of the process for such estimates.

[^direct_estimates]: From $T$ observations, the magnitude of a difference in data indices is no more than $T-1$.  Yet the needed estimates include $\gamma_X(T + u - 1 - j)$, with $j \in [0, T-1]$ and $u \ge 1$.  For $j = 0$ and $u = 1$ we have $\gamma_X(T)$, which exceeds the limits of direct estimation.
...
$$
\begin{align}
  \tilde{\mathcal{N}} (u \; | \; \mathcal{p} (t_f)) &= E \left \{ \mathcal{N} (u + t_f) \; | \; \mathcal{N} (s), \; s \le t_f \right \}
\end{align}
...
$$
\begin{align}
  \hat{\mathcal{N}} (u \; | \; \mathcal{p} (t_f)) &= \tilde{\mathcal{N}} (u \; | \; \mathcal{p} (t_f))
\end{align}
...
Since $\pi_0 = 1$ the last result can also be expressed as an infinite AR model.
...
$$
\begin{align}
  \tilde{\mathcal{N}} (u \; | \; \mathcal{p}(t_f)) &= \sum_{\nu = 0}^\infty \; \psi_\nu \; E \left \{ W(u + t_f - \nu) \; | \; \mathcal{N} (s), \; s \le t_f \right \}
\end{align}
...
$$
\begin{align}
  \tilde{\mathcal{N}} (u \; | \; \mathcal{p} (t_f) &= - \sum_{\nu = 1}^{u - 1} \; \pi_\nu \; \tilde{\mathcal{N}} (u - \nu \; | \; \mathcal{p} (t_f)) \; - \; \sum_{\nu = u}^\infty \; \pi_\nu \; \mathcal{N} (u - \nu + t_f)
\end{align}
$$ {#eq-predictor-inv-form-u}

### Predictor Residuals

We now examine the residuals from the linear least squares predictors defined above.  Let $R(\cdot \; | \; \mathcal{p} (t_f))$ denote the residuals of the predictors.

$$
\begin{align}
  R(k \; | \; \mathcal{p} (t_f)) &= \mathcal{N} (k + t_f) \; - \; \tilde{\mathcal{N}} (k \; | \; \mathcal{p} (t_f)), \quad k \ge 1
\end{align}
$$ {#eq-predictor-resid-defn}

Combining @eq-predictor-inv-form-u and @eq-predicted-AR-infty we can express process $R(u \; | \; \mathcal{p} (t_f))$ for $u \ge 1$ in a finite auto-regressive form (that expands with $u$).

$$
\begin{align}
  R(u \; | \; \mathcal{p} (t_f)) &= \mathcal{N} (u + t_f) \; - \; \tilde{\mathcal{N}} (u \; | \; \mathcal{p} (t_f)) \\ 
  &= W(u + t_f) - \sum_{\nu = 1}^{u - 1} \; \pi_\nu \; R(u - \nu\; | \; \mathcal{p} (t_f))
\end{align}
$$ {#eq-predictor-resid-AR}

Similarly, combing @eq-predicted-causal-form and @eq-predictor-causal-form-u we can express $R(u \; | \; \mathcal{p} (t_f))$ as the following finite moving average of the white noise terms (where the number of terms again increases with $u$).
...
From this last result we see that the residual random variable $R(u \; | \; \mathcal{p} (t_f))$ has zero mean.

$$
\begin{align}
  E\{ R(u \; | \; \mathcal{p} (t_f)) \} &= 0
\end{align}
$$ {#eq-e-predictor-resid-0}

Also note that $R(u \; | \; \mathcal{p} (t_f))$ and $R(u + k \; | \; \mathcal{p} (t_f))$ are correlated in general, for $u \ge 1$ and $k \ge 0$. 

$$
\begin{align}
  E \left \{ R(u \; | \; \mathcal{p} (t_f)) \times R(u + k \; | \; \mathcal{p} (t_f)) \right \} &= \sigma_W^2 \; \sum_{\nu = 0}^{u - 1} \; \psi_\nu \; \psi_{\nu + k}
\end{align}
$$ {#eq-cov-predictor-resid}

Setting $k = 0$ we obtain the following variance of $R(u\; | \; \mathcal{p} (t_f))$.

$$
\begin{align}
  E\{ R(u\; | \; \mathcal{p} (t_f))^2 \} &= \sigma_W^2 \; \sum_{\nu = 0}^{u - 1} \; \psi_\nu^2
\end{align}
...
$$
\begin{align}
  \lim_{u \rightarrow \infty} E\{ R(u\; | \; \mathcal{p} (t_f))^2 \} &= \sigma_W^2 \; \sum_{\nu = 0}^\infty \; \psi_\nu^2
\end{align} = \sigma_\mathcal{N}^2
$${#eq-lim-predictor-resid-MSE}

### Truncated Predictors

We've expressed the linear predictor $\tilde{\mathcal{N}} (u \; | \; \mathcal{p}(t_f))$ as an infinite moving average (@eq-predictor-causal-form-u) and, alternatively, as an infinte auto-regression (@eq-predictor-inv-form-u).
...
\begin{align}
  ...
  \\   
  \tilde{\mathcal{N}} (u \; | \; \mathcal{p} (t_f))
  ...
  \\ 
  \hat{X} (u \; | \; \mathcal{p} (t_f)) 
     \quad & \quad \text{predicts } X(u + t_f) \text{ as affine function of } \{ X(t) \}_{t \in \mathcal{p} (t_f)} \\ 
  \\ 
  \hat{X} (u \; | \; \tau_T) 
     \quad & \quad \text{predicts } X(u + t_f) \text{ as affine function of } \{ X(t) \}_{t \in \tau_T}
\end{align}
$$
