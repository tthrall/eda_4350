...
The following $ARIMA(0, 1, 1)$ model has been applied to many economic time series.[^txt-EWMA-xmpl]
...
The linear prediction of the current value based on past values is thus the same exponential smoothing of past values.[^W-given-past-X]

[^W-given-past-X]: We assume here that $E \left \{ W(t) \; | \; \{ X(s) \}_{s < t} \right \} = 0$.
...
The predictor $\hat{X}(t \; | \; \wp (t-1))$ is called the _one step ahead linear predictor_.  We now have the following linear recursion formula.
...
Now, based on the $IMA(1, 1)$ model we have [^empty-sum-is-0]

[^empty-sum-is-0]: In the case $u = 1$ the sum is over an empty set of indices $\nu$ such that $\nu \ge 1$ and $\nu \le 0$.  We define such empty sums to be zero.
