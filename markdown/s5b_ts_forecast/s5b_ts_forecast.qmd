...
We begin with simple exponential smoothing (SES), which entails the level component $\ell(\cdot)$ alone.  We now combine results from the previous section and change parameters by setting $\alpha = 1 - \lambda$, with $\alpha$ restricted to the open unit interval.  Then we have the following linear recursion for the one step ahead predictor.
...
Now setting aside these boundary conditons for the moment, the state-space form of this linear recursion is as follows.
...
where $\ell (t)$ predicts $X(t + 1)$ based on $\{ X(s) \}_{s \le t}$.  We model the difference $X(t) - \ell (t - 1)$ as a Gaussian white noise process, $W(t)$.
...
The white noise term $W(t)$ is also called an error term, and this state space model is said to have _additive error_.  Equivalently, the level component $\ell (\cdot)$ is said to be additive.
