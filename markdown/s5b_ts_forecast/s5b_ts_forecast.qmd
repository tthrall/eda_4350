...
[^txt-ARMA_p-q-inv]: See @shumway2025 [page 97].
...
[^direct-estimates]: From $T$ observations, the magnitude of a difference in data indices is no more than $T-1$.  Yet the needed estimates include $\gamma_X(T + u - 1 - j)$, with $j \in [0, T-1]$ and $u \ge 1$.  For $j = 0$ and $u = 1$ we have $\gamma_X(T)$, which exceeds the limits of direct estimation.
...
Now, restricting the predictor to the available data by using the $ARMA(p, q)$ model, we recursively define the following "truncated" predictor, $\breve{\mathcal{N}} (u \; | \; \tau_T)$.
