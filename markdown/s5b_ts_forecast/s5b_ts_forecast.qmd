...
That is, the current one step ahead predictor is a convex combination of the most recent observation and the preceding one step ahead predictor.  The truncated predictor based on the data follows a similar recursion.
...
$$
\begin{align}
  Var \left\{ \breve{X}(u + t_0 \; | \; \tau_u) \right \} &= \sigma_W^2 \; \left \{ 1 + (u-1)\; (1 - \lambda)^2 \right \} \quad \text{for } 0 < u < T
\end{align}
$$
