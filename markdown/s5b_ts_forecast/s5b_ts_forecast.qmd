...
@fig-EWMA-plus-wn shows a simulation of the IMA process $X(\cdot)$, along with one step ahead exponential smoothing of past values, and the $IMA(1, 1)$ forecast of future values.  We set $\lambda =$ `r EWMA_sim_params$ lambda`, equivalent to an EWMA smoothing parameter of $\alpha = 1 - \lambda =$ `r EWMA_sim_params$ alpha`.
