...
Due to the normality of $\mathcal{N}(\cdot)$ this conditional expectation $\tilde{\mathcal{N}}$ coincides with the linear least squares predictor $\hat{\mathcal{N}}$, which is the relevant predictor for non-normal processes.
...
We now examine the residuals from the linear least squared predictors defined above.  For fixed $u$ the predicted variable is $\mathcal{N} (u + t_f)$, which we now express in both causal and inverted forms.
...
We now re-express @eq-predicted-inv-form as an infinite AR model.
...
Let $R(\cdot)$ denote the residual of the predictors.

$$
\begin{align}
  R(k) &= \mathcal{N} (k + t_f) \; - \; \tilde{\mathcal{N}} (k \; | \; \mathcal{p} (t_f), \quad k \ge 1
\end{align}
$$ {#eq-predictor-resid-defn}

Combining @eq-predictor-inv-form-u and @eq-predicted-AR-infty we can express process $R(k)$ for $k \ge 1$ in an expanding but finite auto-regressive form.

$$
\begin{align}
  R(u) &= \mathcal{N} (u + t_f) \; - \; \tilde{\mathcal{N}} (u \; | \; \mathcal{p} (t_f) \\ 
  &= W(u + t_f) - \sum_{\nu = 1}^{u - 1} \; \pi_\nu \; R(u - \nu)
\end{align}
$$ {#eq-predictor-resid-AR}

Similarly, combing @eq-predicted-causal-form and @eq-predictor-causal-form-u we can express $R(\cdot)$ as the following finite but expanding moving average of the white noise terms.
...
For fixed $u$ the residual random variable $R(u)$ has zero mean and the following finite variance.

$$
\begin{align}
  E\{ R(u)^2 \} &= \sigma_W^2 \; \sum_{\nu = 0}^{u - 1} \; \psi_\nu^2
\end{align}
$$ {#eq-predictor-resid-MSE}

As $u \rightarrow \infty$ the residual variance approaches the variance of the original process $\mathcal{N} (\cdot)$.

$$
\begin{align}
  \lim_{u \rightarrow \infty} E\{ R(u)^2 \} &= \sigma_W^2 \; \sum_{\nu = 0}^\infty \; \psi_\nu^2
