...
Since the multiplicative inverse of the polynomial $1 - \lambda z$ is a geometric series in $\lambda z$ we also have the following inverted form of the model.
...
That is, the current value $X(t)$ is an exponential smoothing of all past values plus $W(t)$, the white noise term.  The linear prediction of the current value based on past values is thus the same exponential smoothing.
...
To calculate forecast error bounds we require $\psi$ coefficients from the causal form of the model. In this example they are the power series coefficients of the rational function $\psi(z) = (1 - \lambda \; z)(1 - z)^{-1}$.  This gives 
...
$$
\begin{align}
  Var \left\{ \hat{X}(u \; | \; \wp (t_f)) \right \} &= \sigma_W^2 \; \left \{ 1 + (u-1)\; (1 - \lambda)^2 \right \} 
\end{align}
$$

@fig-EWMA-plus-wn shows a simulation of the IMA process $X(\cdot)$, along with the one step ahead exponential smoothing of past values.  We set $\lambda = 0.8$, equivalent to an EWMA smoothing paramter of $\alpha = 1 - \lambda = 0.2$.

```{r}
#| label: fig-EWMA-plus-wn
#| fig-cap: "IMA(1, 1) with exponenetial smoothing (alpha = 0.2)"
#| eval: true

set.seed(666)

# λ = 1 − α = .8
x <- astsa::sarima.sim(d = 1, ma = -0.8, n = 100)
x.ima <- stats::HoltWinters(x, beta=FALSE, gamma=FALSE)

# λˆ = 1 − αˆ ≈ .81
# Smoothing parameter: alpha: 0.1853541

# plots observed and fitted
plot(
  x.ima, 
...
This introduction does not address the building and evaluation of time series models, leaving that for another time.  Here are some additional resources for those interested.
  ...
  - [Regression and Time Series Primer](https://dsstoffer.github.io/Rtoot.html#regression-and-time-series-primer) for a more detailed introduction to the topics presented here.
  - [Fun with asta](https://github.com/nickpoison/astsa/tree/master/fun_with_astsa) introduces to `R` package `astsa`.
