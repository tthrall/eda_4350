...
Now, the $AR(p)$ model is defined to be that of a _stationary_ process $X(t)$, which constrains the coefficients of polynomial $\phi(\cdot)$.  Additional requirements that the model be _causal_[^causal] and _invertible_[^invertible] restrict attention to polynomials $\phi(\cdot)$ whose roots are all greater than unity in magnitude.  This ensures that the function $\phi (z)^{-1}$ is well defned and has a power series expansion for for $|z| \le 1$.
...
The probability distributions of the two processes $X(\cdot)$ and $Y(\cdot)$ are identical[^MA1_equiv], so anyone attempting to identify the respective models could not distinguish them based on their realizations as time series data.

[^MA1_equiv]: To see the equivalence of the two models, set $U(t) = 5 V(t-1)$, and then reverse time.
...
And here is the corresponding autocorrelation function (ACF).
...
As for the PACF, recall that it is defined to coincide with the ACF at lag 1.  
...
$$
\begin{align}
  \rho_X (u) &= corr \left ( X(t + u), X(t) \right ) \\ 
  &= 
  \begin{cases}
    1 & \text{ for } u = 0 \\ 
