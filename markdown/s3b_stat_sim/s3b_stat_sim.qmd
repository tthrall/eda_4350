...
Statistical simulation is used to model the behavior of complex systems and analyze potential outcomes of various scenarios by generating random data from specified distributions. Simulations augment analytic solutions, enabling researchers to explore different possibilities and assess alternative methods.

Here are some of the uses of statistical simulation.
...
Here are two simulations, each starting at $X(0) = 0$, but with $\delta \in \{ 0.1, 1.0 \}$, respectively.  Recall that $| X(t+1) - X(t) | \le \delta$. Thus the first trajectory is much smoother than the second.
...
nrm_rw_wide <- tibble::tibble(
  x_01  = gen_nrw(delta = 0.1), 
  x_10  = gen_nrw(delta = 1.0), 
  t_idx = 1:length(x_01)
) |> 
  dplyr::select(t_idx, x_01, x_10)
...
#| fig-cap: "X(t) with small and large jumps"

g_nrm_rw <- nrm_rw_long |> 
  dplyr::group_by(d_chr) |> 
  dplyr::rename(
    time = t_idx, 
    z = value, 
    delta = d_chr
  ...
  ggplot2::ggplot(mapping = aes(
    x = time, y = z, fill = delta, color = delta
  )) + 
...
Next we see histograms of the generated values.  For the series with $\delta = 0.1$, more time would have been needed to traverse a greater portion of the normal distribution values.

```{r}
#| label: fig-nrw-hist
#| fig-cap: "Histograms of X(t) with small and large jumps"
...
The extremes of the two random walks are similar, but the first random walk was confined to a narrower range of values centered at `-1`.
