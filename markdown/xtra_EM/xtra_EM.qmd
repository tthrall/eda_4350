...
params:
  ...
  file_prefix: "xtra_EM"
abstract: 
  "The EM algorithm is introduced and demonstrated.  <br><br>"
---
...
The figure below represents the duration of eruptions ("duration") and the interval between eruptions ("delay")[^delay] of the "Old Faithful" geyser in Yellowstone National Park, Wyoming from August 1 to August 15, 1985.[^faithful-data]
...
```{r}
#| label: fig-faithful-tbl
#| fig-cap: "Old Faithful eruptions: delay and duration (minutes)"
...
g_faithful_tbl <- faithful_tbl |> 
  ...
  ggplot2::geom_point() + 
  ggplot2::labs(title = "Old Faithful eruptions: delay and duration") + 
  ggplot2::xlab("delay (minutes") + 
  ggplot2::ylab("duration (minutes")
g_faithful_tbl
```

### Gausian Mixture Model

The figure shows two clusters of data points.[^faithful-chambers] Here is probability model in which the delay and duration of each eruption follows one of two possible bivariate normal distributions depending on the group, $G$, to which it belongs.

[^faithful-chambers]: Old Faithful eruptions are conjectured to occur in 2 distinct temporal patterns due to the presence of an upper and lower chamber beneath the vertical column (tube) that forms the geyser.
...
In the EM framework, we iteratively group-membership, $G = g \in \{ 1, 2 \}$, to each data point.  Based on this set of assignments we update the parameter estimates (the "E" step of the EM algorithm). The new set of parameter estimates yields a new estimate of the likelihood function. We then re-assign cluster-membership to maximize the updated likelihood estimate (the "M" step). The iteration terminates once the magnitude of changes falls below a prescribed threshold.
...
2.  For each data point determine whether the `delay` value is less than the median of all observed `delay` values. Similarly determine whether the `duration` value is less than the median of all observed `duration` values.

3.  Based on these inequalities, categorize each (`delay`, `duration`) data point as belonging to one of four groups: (lower, lower), (lower, upper), (upper, lower), (upper, upper).

4.  Restrict attention to the (lower, lower), and (upper, upper) groups. Within this restricted set of data points calculate the sample averages and covariance matrices for each of the two groups. Use these as initial estimates of $(\mu_{\bullet}^{(1)}, \mu_{\bullet}^{(2)})$ and $(\Sigma^{(1)}, \Sigma^{(2)})$.
...
```{r}
#| label: tbl-faithful-4-smy
#| tbl-cap: "Summary statistics for initial groups of data points"
...
faithful_2 <- faithful_4 |> 
  ...
  dplyr::mutate(
    g = dplyr::if_else(delay_low, 1L, 2L)
  ) |> 
  dplyr::select(g, delay, duration)
```
...
initial_est <- faithful_4_smy |> 
  ...
  dplyr::select(
    g, count, p, delay_mean:dd_cov
  )
```

```{r initial_est__kable}
#| label: tbl-initial-est
#| tbl-cap: "Initial parameter estimates"

initial_est |> 
  dplyr::select(g, count, delay_mean:dd_cor) |> 
  knitr::kable(
...
```{r}
#| label: fig-faithful-2
#| fig-cap: "Old Faithful eruptions: initial normal estimates"

delay_median    <- faithful_tbl$ delay    |> median()
duration_median <- faithful_tbl$ duration |> median()

g_faithful_2 <- faithful_2 |> 
  dplyr::mutate(g = forcats::as_factor(g)) |> 
  dplyr::group_by(g) |> 
  ggplot2::ggplot(mapping = aes(
    x = delay, y = duration, colour = g
  )) + 
  ggplot2::geom_point(
    mapping = aes(colour = g, shape = g), 
    show.legend = TRUE
  ) + 
  ggplot2::geom_vline(xintercept = delay_median) + 
  ggplot2::geom_hline(yintercept = duration_median) + 
  ggplot2::stat_ellipse(level = 0.5) + 
  labs(
...
f_tag_tbl <- faithful_tbl |> 
  ...
  dplyr::mutate(
    ...
    g = dplyr::if_else(md_1 < md_2, 1L, 2L)
  )
f_tag_tbl <- f_tag_tbl |> 
  dplyr::ungroup()
```
...
```{r}
#| label: fig-f-tag-tbl
#| fig-cap: "Old Faithful: inital clustering of all points"

g_f_tag_tbl <- f_tag_tbl |> 
  dplyr::mutate(g = forcats::as_factor(g)) |> 
  dplyr::group_by(g) |> 
  ggplot(mapping = aes(
    x = delay, y = duration, group = g
  )) + 
  geom_point(
    aes(
      colour = g, 
...
```{r}
#| label: tbl-f-tag-smy
#| tbl-cap: "Summary statistics for initial clusters of data points"

f_tag_smy |> 
  dplyr::select(g:dd_cor) |> 
  knitr::kable(
...
f_em <- faithful_tbl |> 
  ...
  dplyr::mutate(
    g = dplyr::if_else(comp.1 > comp.2, 1L, 2L)
  )
```
...
```{r}
#| label: tbl-f-em-smy
#| tbl-cap: "Summary statistics for final groups of data points"

f_em_smy |> 
  dplyr::select(g, count:dd_cor) |> 
  knitr::kable(
...
```{r}
#| label: fig-f-em
#| fig-cap: "Old Faithful eruptions: final normal estimates"
...
```{r}
#| label: fig-grp-per-cum-delay
#| fig-cap: "Group 1 posterior probability over cumulative delay (days)"

g_grp_per_cum_delay <- f_em |> 
  # cumulative delay in days
  dplyr::mutate(
    cum_delay = cumsum(delay) / (60 * 24), 
    g = forcats::as_factor(g)
  ) |> 
  ggplot2::ggplot(mapping = aes(
    x = cum_delay, y = comp.1, color = g, fill = g)) + 
  ggplot2::geom_point(
    mapping = aes(colour = g, shape = g), 
    show.legend = TRUE) + 
  ggplot2::labs(
    title = "Group 1 posterior probability over cumulative delay (days)") + 
  ggplot2::xlab("cumulative delay (days)") + 
  ggplot2::ylab("group 1 posterior probability")
g_grp_per_cum_delay
```
