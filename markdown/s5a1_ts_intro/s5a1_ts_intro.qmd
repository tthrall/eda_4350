...
[^stationary]: The assumption of second-order stationarity suffices for most time series models used in practice, but in other cases one might need to assume _strict stationarity_, which means that all finite-dimensional distributions of the random process are unchanged by time shifts.
...
The aims and methods of data analysis can be categorized in various ways, for example, confirmatory data analysis and exploratory data analysis.  For the moment we consider the complementary aims of decision support and scientific understanding.

### Decision Support

Financial management operates at markedly different time scales, from algorithms that act in real time to decisions to invest in multi-year projects.  The financial stakes warrant substantial investment in all aspects of the technology, including time series modeling.  To be useful the models must be as precise and as reliable as possible.  The particular application may determine the balance struck between precision and reliability.
...
The next figure shows a 5-year forecast based on a time-domain model (seasonal auto-regressive, moving-average model).
...
We leave the development of such models to a separate technical note.  The point here is that such projections are both valued and scrutinized when offered as an argument for climate-related decisions.
