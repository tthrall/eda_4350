...
This technical note gives examples of data in which successive observations are statistically dependent and are typically indexed by time.[^time_index] Time series data sampled at regular intervals (daily stock prices, for example) are the most common case, followed by point process data representing the occurrence of events (earthquakes, for example) at random times.

[^time_index]: In rare cases time series methods are applied  to data indexed not by time but rather by some other measure, e.g. distance.  See for example, D.J. Thompson (1977) "Spectrum Estimation Techniques for Characterization and Development of WT4 Waveguide" <https://doi.org/10.1002/j.1538-7305.1977.tb00591.x>
...
[^djia_return]: The return is here calculated as $log_e$ of the ratio, current day's closing price divided by that of the preceding day.  The median value is about 6 basis points.
...
\end{align}
$$
Consequently, second-order stationarity enables one to define and estimate the following _auto-covariance_ function.
...
The auto-covariance function is often rescaled to yield the _autocorrelation_ function $\rho_{\bullet, \bullet}(\cdot)$.
...
In this case the 5-year moving average operation can be expressed as follows.[^ma_5]

[^ma_5]: The example illustrates a simple moving average.  More generally a weighted moving average may have non-negative coefficients that sum to unity.
...
Here $X_\bullet (\cdot)$ is a two-component random process representing annual global averages of land and ocean temperatures.  This 5-year moving average is an example of a linear, time-invariant _filter_, having the following general form.
...
There are a number of ways to categorize types of data analysis.  One important distinction is between confirmatory and exploratory data analysis.  For present purposes we consider the complementary aims of decision support and scientific understanding.
...
The auto-correlation function is thus[^rho_ma_1] 
...
That is, ignoring the autocorrelation of $X(\cdot)$ would lead to erroneous calculations of the variance of $S_{X, T}$ and $\hat{\mu}_X$.

### Effective Sample Size (ESS)

The magnitude of such erroneous calculations can be represented by a quantity called the _effective sample size_ (ESS).  The calculation of ESS varies with application[^astsa_ESS], but here's the basic idea in the present context.  Consider a finite sequence of _independent_ draws from the distribution of the random variable $X(t_0)$, thereby producing a sample of independent observations.  Define the effective sample size, $N_d$, to be the number of draws required to produce the same variance of the resulting sample mean as that of the auto-correlated case.
...
This technical note is intended as an overview of time series analysis based on the textbook cited below by Shumway and Stoffer, along with the supporting `R` package `astsa`.  The reader should feel free to select the presented examples of greatest interest, skipping others to save time.  Time-domain and frequency-domain methods are mentioned only in the broadest terms, leaving details for other presentations.
...
```{r s5a1_books_tbl__save}
# save file only when updated
save_to_file <- FALSE
if (save_to_file) {
  # RDS format
  s5a1_books_tbl |> readr::write_rds(here::here(
    "data", "retain", "s5a1_books_tbl.rds"
  ))

  # TSV format
  s5a1_books_tbl |> readr::write_tsv(here::here(
    "data", "retain", "s5a1_books_tbl.txt"
  ))
}
```

```{r s5a1_books_tbl__kable}
s5a1_books_tbl |> 
  list_links(a_idx = 3L) |>
  knitr::kable(
    format    = "html",
...
```{r s5a1_articles_tbl}
s5a1_articles_tbl <- tibble::tribble(
  ~title, ~link,
...
```{r s5a1_articles_tbl__save}
# save file only when updated
save_to_file <- FALSE
if (save_to_file) {
  # RDS format
  s5a1_articles_tbl |> readr::write_rds(here::here(
    "data", "retain", "s5a1_articles_tbl.rds"
  ))

  # TSV format
  s5a1_articles_tbl |> readr::write_tsv(here::here(
    "data", "retain", "s5a1_articles_tbl.txt"
  ))
}
```

```{r s5a1_articles_tbl__kable}
s5a1_articles_tbl |>
  list_links() |>
  knitr::kable(
    format    = "html",
...
```{r s5a1_symbol_tbl}
s5a1_symbol_tbl <- tibble::tribble(
  ~tag, ~symbol, ~dscr, 
...
```{r s5a1_symbol_tbl__save}
# save file only when updated
save_to_file <- FALSE
if (save_to_file) {
  # RDS format
  s5a1_symbol_tbl |> readr::write_rds(here::here(
    "data", "retain", "s5a1_symbol_tbl.rds"
  ))

  # TSV format
  s5a1_symbol_tbl |> readr::write_tsv(here::here(
    "data", "retain", "s5a1_symbol_tbl.txt"
  ))
}
```

```{r s5a1_symbol_tbl__kable}
s5a1_symbol_tbl |>
