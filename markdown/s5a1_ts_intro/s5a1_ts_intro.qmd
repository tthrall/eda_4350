...
Here $\mathcal{B}$ denotes the _back-shift_ operator that shifts time by one unit, so that $\mathcal{B}^s$ shifts time by $s$ units.  Then second-order stationarity can be expressed as follows.
...
The 5-year moving average above can be expressed as follows.
...
In signal-processing applications the linear, time-invariant filter $a_{\bullet, \bullet} (\cdot)$ may be designed to extract certain types of signals that are contaminated by noise.

A moving average operation smooths the time series on which it operates.  That is, it allows low-frequency components to pass, while diminishing high-frequency components.  Therefore the moving average operation is categorized as a _low-pass_ filter.
