...
```{r gt_ref_land}
# gtemp_land 
#   check 1991-2020 average temperature
gt_ref_land <- astsa::gtemp_land |> 
  tsibble::as_tsibble() |> 
  dplyr::filter(index >= 1991, index <= 2020)

# gt_ref_land$value |> summary()
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  # 0.360   0.815   1.120   1.194   1.570   2.500
```

```{r gt_ref_ocean}
# gtemp_ocean 
#   check 1991-2020 average temperature
gt_ref_ocean <- astsa::gtemp_ocean |> 
  tsibble::as_tsibble() |> 
  dplyr::filter(index >= 1991, index <= 2020)

# gt_ref_ocean$value |> summary()
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # 0.2600  0.3700  0.4350  0.4777  0.5525  0.8300
```

```{r gt_ref_both}
# gtemp_both 
#   check 1991-2020 average temperature
gt_ref_both <- astsa::gtemp_both |> 
  tsibble::as_tsibble() |> 
  dplyr::filter(index >= 1991, index <= 2020)

# gt_ref_both$value |> summary()
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  0.3000  0.5175  0.6750  0.6993  0.8425  1.3500
```
...
Subtracting off this mean function, the remaining residual random process, $X_\bullet (t) - m_\bullet (t)$ , is often assumed to be __second-order stationary__.  That is, the covariance structure of the given random process is assumed to be the same as that of the time-shifted process $Y_\bullet (t) = X_\bullet (t + s)$ for any shift $s$ in time.  Based on this assumption one can define and estimate the following auto-covariance function.
...
In the example above of Johnson and Johnson stock prices, we saw that a log transformation, $Y(t) = \log_e (X(t))$, could be more closely fit by a linear trend.  (We set aside for now consideration of such transformations on the variance of the time series.)

In addition to the data transformations used on independent observations, we often want to average or otherwise filter time series data with respect to time.

The figure below shows a 5-year moving average of the global temperature data previously shown.  This operation smooths out local fluctuations to show the overall trend more clearly.
