...
Subtracting off this mean function, the remaining residual random process, $X_\bullet (t) - m_\bullet (t)$ , is often assumed to be *second-order stationary*.  That is, the covariance structure of the given random process is assumed to be the same as that of the process $Y_\bullet (t) = X_\bullet (t + s)$ for any shift $s$ in time of the given process.  Based on this assumption one can estimate the auto-covariance function.
