...
Setting $b = a$, we obtain $f_{a, a} (\lambda)$, called the _power spectrum_ of $X_a (\cdot)$.

If $b \ne a$ then $f_{a, b} (\lambda)$ is called the _cross-spectrum_ of $\{ X_a (\cdot), X_b (\cdot) \}$.  The _coherency_ of $X_a(\cdot), X_b(\cdot)$ at frequency $\lambda$, denoted $R_{a, b} (\lambda)$, is the following normalization of the cross-spectrum.
...
@fig-hnl-nyc-cp-zoom shows the two cities to be perfectly coherent and in phase at frequenccies near the annual cycle, as expected.
