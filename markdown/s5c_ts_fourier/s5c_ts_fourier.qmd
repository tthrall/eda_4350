...
g_sunspot_spec <- sunspot_spec_tbl |> 
  ...
  ggplot2::labs(
    ...
    subtitle = "Estimated spectral density function") + 
  ggplot2::xlab("freq = cycles per month") + 
  ggplot2::ylab("spectral density")
g_sunspot_spec
```

@fig-sunspot-spec_log10 gives a closer view of the spectral density function at the lowest observable frequencies.  The peak in the spectrum occurs at frequency `r round(peak_freq, 3)` cycles per month corresponding to a wavelength of `r round(peak_wave_yr, 1)` years.  As previously mentioned, the gaps between peak values of sunspot numbers vary somewhat, which is part of the reason for the breadth of this peak in the spectral density (the other part being the unavoidable periodogram-averaging required to lower the variance of the estimated spectral density function.)

```{r}
#| label: fig-sunspot-spec_log10
#| fig-cap: "Sunspot spectral density: zoom-in view"
...
g_sunspot_spec <- sunspot_spec_tbl |> 
  ...
  ggplot2::geom_line() + 
  ggplot2::labs(
    title = "Sunspot spectral density function", 
    subtitle = "zoom-in view") + 
  ggplot2::xlab("freq = cycles per month") + 
  ggplot2::ylab("log_10( spectral density )")
g_sunspot_spec
```
...
@fig-hnl-nyc-spec below shows (on a log-10 scale) estimated spectral densities of daily temperatures in Honolulu and New York City.

```{r}
#| label: fig-hnl-nyc-spec
#| fig-cap: "City temperatures: estimated spectral densities"
...
g_hnl_nyc_spec <- hnl_nyc_spec_long |> 
  ...
  ggplot2::facet_grid(
    ...
    scales = "free_y") + 
  ggplot2::labs(
    title = "HNL, NYC spectral densities", 
...
\begin{align}
  ...
  \\ 
  f_{\bullet, \bullet} (\lambda) 
     \quad & \quad \text{spectral density matrix at frequency } \lambda \\ 
  \\ 
  \phi(\cdot)
