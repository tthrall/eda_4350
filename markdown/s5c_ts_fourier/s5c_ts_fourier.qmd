...
The discrete Fourier transform (DFT) is the restriction of the finite Fourier transform to a grid of frequencies spanning the interval $[0, 2 \pi)$, or some other half-open interval of length $2 \pi$.

To simplify notation we consider a univariate process $X(\cdot)$.  We also index observed values as $\{ X(0), X(1), \ldots, X(T-1) \}$.  Now we define the DFT of this time series as follows.
...
Now we define $\hat{f}_{\bullet, \bullet} (\lambda)$ as follows.
...
Under the previously mentioned regularity conditions for $X_{\bullet, \bullet} (\cdot)$, $\hat{f}_{\bullet, \bullet} (\lambda)$ is a consistent estimator and is asymptotically normal.[^book-consistent-spec-dens-est]

[^book-consistent-spec-dens-est]:  See @brillinger2001 [sec. 5.6] for further details.

## Examples {#sec-spec-examples}
