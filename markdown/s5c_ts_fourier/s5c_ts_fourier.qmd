...
$$
\begin{align}
  f_{X, X} (\lambda) &= \left | \psi(e^{-i \lambda}) \right |^2 \; \sigma_W^2 (\lambda)
\end{align}
$$ {#eq-spec-dens-causal}
...
\begin{align}
  ...
  \text{ so that } \\ \\ 
  \psi (z) &= \frac{1 + \theta_1 z}{1 - \theta_1 z} \\ 
  &= (1 + \theta_1 z) \; \sum_{\nu = 0}^\infty \phi_1^\nu \times z^\nu \\ 
  &= 1 \; + \; \sum_{k = 1}^\infty (\phi_1^k + \theta_1 \; \phi_1^{k - 1}) \times z^k
\end{align}
$$ {#eq-ARMA-1-1-causal}

### Simulations from ARMA models
...
```{r}
#| label: ARMA-sim-spec-dens

spec_W_lst <- sim_tbl$ W |> 
  spec.pgram(detrend = FALSE, plot = FALSE, taper = 1/4)
...
spec_sim <- tibble::tibble(
  ...
  W = sim_tbl$ W |> spec_strip(), 
  X = sim_tbl$ X |> spec_strip(), 
  Y = sim_tbl$ Y |> spec_strip(), 
  Z = sim_tbl$ Z |> spec_strip()
)
...
@fig-ARMA-sim-spec-dens shows the estimated density function of each simulated ARMA process.

```{r}
#| label: fig-ARMA-sim-spec-dens
