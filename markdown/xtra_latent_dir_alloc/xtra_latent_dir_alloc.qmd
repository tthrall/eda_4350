...
The data set to be analyzed is a _corpus_ (set) $\mathcal{D}$ of documents, with each _document_ $d \in \mathcal{D}$ consisting of a sequence of words, that may include multiple instances of the same term.
...
The elements of $V$ are referred to as _terms_.  We will use _word_ to mean an occurrence of a term within the corpus.  With this distinction, the total number of words (term occurrences) in the corpus typically exceeds the number of distinct terms.[^token-NLP] [^term-v-word]
