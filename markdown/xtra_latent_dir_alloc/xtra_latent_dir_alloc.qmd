...
if (extract_from_DTM && topicmodels_loaded) {
  ...
  # save output components as TSV files
  # but save only as needed
  save_file <- TRUE
  if (save_file) {
    
    # z: topic assignment of successive words across corpus
    af_bbb_gs_z <- af_bbb_gs@ z |> 
      tibble::as_tibble_col(column_name = "z")
    af_bbb_gs_z |> readr::write_tsv(here::here(
      "data", "retain", "af_bbb_gs_z.txt"
    ))
    
    # conc: initial concentration parameters
    af_bbb_gs_conc <- tibble::tribble(
      ~param, ~value, 
      "alpha", af_bbb_gs@ alpha,
      "delta", af_bbb_gs@ control@ delta)
    af_bbb_gs_conc |> readr::write_tsv(here::here(
      "data", "retain", "af_bbb_gs_conc.txt"
    ))
    
    # terms: unique words across the corpus
    af_bbb_gs_terms <- tibble::tibble(
      tdx  = 1:length(af_bbb_gs@ terms), 
      term = af_bbb_gs@ terms)
    ...
    # Note: saved tibble is transpose of the K-by-N beta matrix
    af_bbb_gs_beta <- af_bbb_gs@ beta |> 
      t() |> tibble::as_tibble()
    names(af_bbb_gs_beta) <- paste0("topic_", 1:2)
    af_bbb_gs_beta |> readr::write_tsv(here::here(
      "data", "retain", "af_bbb_gs_beta.txt"
    ))
    
    # gamma: prob(topic | doc), a D-by-K matrix
    af_bbb_gs_gamma <- af_bbb_gs@ gamma |> 
      tibble::as_tibble()
    names(af_bbb_gs_gamma) <- paste0("topic_", 1:2)
    af_bbb_gs_gamma |> readr::write_tsv(here::here(
      "data", "retain", "af_bbb_gs_gamma.txt"
    ))
    
    # dw_assign: topic assignment per (doc, term)
    af_bbb_gs_dw_assign <- tibble::tibble(
      doc_idx   = af_bbb_gs@ wordassignments$ i, 
      trm_idx   = af_bbb_gs@ wordassignments$ j, 
      topic_idx = af_bbb_gs@ wordassignments$ v |> as.integer()
    )
...
af_bbb_word_tally <- tibble::tibble(
  ...
  elt  = c("word", "z", "wt", "count", "af_bbb_vocab", "terms"), 
  len  = c(
...
The data to be analyzed is a _corpus_ (set) $\mathcal{D}$ of documents, with each _document_ $d \in \mathcal{D}$ consisting of a sequence of _terms_ (words).
...
$$
\begin{align}
  V &= \left \{ v \in d \; | \; d \in \mathcal{D} \right \}
\end{align}
$$ {#eq-doc-equals-word-seq}

A _topic_ $\tau$ is a probability distribution over vocabulary $V$.  Since $V$ consists of a finite number $N = \left | V \right |$ of distinct terms, $\tau$ can be represented as probability vector of length $N$.

The number $K$ of topics (categories) is prescribed for each analysis.  Given $K$ and $\mathcal{D}$, LDA produces a _topic model_ based on a set of $K$ constructed topics.  The model includes a set of probabilities $\beta$
